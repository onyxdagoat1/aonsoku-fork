...
const MUSIC_LIBRARY_PATHS = (process.env.MUSIC_LIBRARY_PATH || '/music')
  .split(',')
  .map((p) => p.trim())
...
// Helper to normalize paths (especially WSL to Windows)
function normalizePath(p) {
  if (!p) return p
  let normalized = path.normalize(p).replace(/\\/g, '/')

  // Convert /mnt/c/... to C:/... if on Windows
...
// Helper to find a file in the library paths
async function getFullFilePath(songPath) {
  const relativePath = songPath.startsWith('/')
    ? songPath.substring(1)
    : songPath

  for (const libraryPath of MUSIC_LIBRARY_PATHS) {
    const normalizedLibrary = normalizePath(libraryPath)
    const fullPath = path.join(normalizedLibrary, relativePath)
  ...
  // Last resort: join with first path anyway for error reporting
  return path.join(normalizePath(MUSIC_LIBRARY_PATHS[0]), relativePath)
}

// Helper function to get Navidrome API credentials
function getNavidromeAuth() {
  if (!NAVIDROME_USERNAME || !NAVIDROME_PASSWORD) {
    console.warn('WARNING: Navidrome credentials (u/p) are missing from .env')
  }
...
async function triggerNavidromeRescan() {
  try {
    ...
    const url = `${NAVIDROME_URL.replace(/\/+$/, '')}/rest/startScan?${params}`
    await axios.get(url)
...
async function getSongFromNavidrome(songId) {
  try {
    ...
    const url = `${NAVIDROME_URL.replace(/\/+$/, '')}/rest/getSong?${params}`

    console.log(
      `Fetching song info from Navidrome: ${url.replace(/u=[^&]+/, 'u=REDACTED').replace(/t=[^&]+/, 't=REDACTED')}`,
    )
    ...
    if (!subsonicResponse) {
      console.error(
        'Invalid response from Navidrome: missing subsonic-response',
      )
      return null
    }
    ...
    if (subsonicResponse.status === 'failed') {
      ...
      console.error(
        `Navidrome API error: code ${error?.code}, message: ${error?.message}`,
      )
      return null
    }
...
function convertToID3Tags(metadata, existingTags = {}) {
  ...
  // Add Yeditor credit to comments if provided
  if (metadata.yeditorName) {
    const editorInfo = `Edited by ${metadata.yeditorName}`
    const currentComment = tags.comment?.text || ''
    if (!currentComment.includes(editorInfo)) {
      tags.comment = {
        text: currentComment
          ? `${currentComment}\n\n${editorInfo}`
          : editorInfo,
...
app.post('/api/update-tags', async (req, res) => {
  try {
    ...
    const songPath = song?.path || req.body.path

    if (!songPath) {
      return res
        .status(404)
        .json({ error: 'Song not found in Navidrome and no path provided' })
    }

    // Construct full file path
    const filePath = await getFullFilePath(songPath)
    ...
    try {
      ...
      return res
        .status(404)
        .json({ error: 'Music file not found on disk or inaccessible' })
    }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      // Construct full file path
      const filePath = await getFullFilePath(song.path)
...
app.listen(PORT, () => {
  ...
  console.log(`Music Libraries: ${MUSIC_LIBRARY_PATHS.join(', ')}`)
})
