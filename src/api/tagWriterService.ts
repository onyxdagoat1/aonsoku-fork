...
const TAG_WRITER_URL =
  import.meta.env.VITE_TAG_WRITER_URL || 'http://localhost:3001'
...
class TagWriterService {
  ...
  async updateCoverArt(
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        throw new Error(
          error.response?.data?.error || 'Failed to update cover art',
        )
      }
      throw error
    }
  ...
  async getCurrentTags(songId: string): Promise<Record<string, unknown>> {
    try {
      const response = await this.client.get<{ tags: Record<string, unknown> }>(
        `/api/get-tags/${songId}`,
      )
  ...
  async triggerRescan(): Promise<{ success: boolean; message: string }> {
    try {
      ...
      if (axios.isAxiosError(error)) {
        throw new Error(
          error.response?.data?.error || 'Failed to trigger rescan',
        )
      }
      throw error
    }
